...
/// The rollback guard in `fix_book_with` asks `well_formed` whether a pass
/// damaged a file, and the reader only parses attributes on request — so a tag
/// carrying the same attribute twice, which XML forbids outright, read back as
/// perfectly well-formed while no reader would open the file. A fixer renaming
/// an attribute onto a name the tag already uses produces exactly that, and the
/// safety net has to see it.
#[test]
fn a_duplicated_attribute_is_not_well_formed() {
    let good = r#"<html><meta name="a" content="A" value="B"/></html>"#;
    assert!(epubfix::markup::well_formed(good).is_ok());

    for bad in [
        r#"<html><meta name="a" content="A" content="B"/></html>"#,
        r#"<html><p class="a" class="b">x</p></html>"#,
    ] {
        assert!(
            epubfix::markup::well_formed(bad).is_err(),
