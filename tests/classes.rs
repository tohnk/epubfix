...
/// An EPUB 3 book whose first chapter has `body`.
fn book3(body: &str) -> Vec<u8> {
    let opf = opf("3.0", "", "", "");
    make_epub(&[
        ("META-INF/container.xml", CONTAINER.as_bytes()),
        ("OEBPS/content.opf", opf.as_bytes()),
...
fn a_nested_anchor_is_unwrapped_and_its_text_kept() {
    ...
    // Two links cannot share one run of text, so the inner destination cannot
    // be kept — but it is named rather than dropped in silence.
    assert!(
        outcome
            .findings
            .iter()
            .any(|f| f.contains("destination of its own") && f.contains("ch1.xhtml")),
        "got {:?}",
        outcome.findings
    );
}
...
fn a_nested_anchor_with_an_id_becomes_a_span_so_links_still_land() {
    ...
    // Nothing was lost, so nothing is reported. This is the shape all three
    // nested anchors in a 179-book library actually have.
    assert!(outcome.findings.is_empty(), "got {:?}", outcome.findings);
}
...
#[test]
fn a_div_inside_a_paragraph_is_split_out() {
    // Word-to-EPUB converters drop a <div> inside a <p> all the time. The
    // paragraph is split around it, which is what a browser has been doing
    // with the file all along.
    let (_, after) = fix(&book2("<p>before<div>block</div>after</p>"));
    let fixed = ch1(&after);
    assert!(
        fixed.contains("<p>before</p><div>block</div><p>after</p>"),
        "{fixed}"
    );
}

#[test]
fn a_nested_paragraph_is_split_out() {
    let (_, after) = fix(&book2("<p>one<p>two</p>three</p>"));
    let fixed = ch1(&after);
    assert!(
        fixed.contains("<p>one</p><p>two</p><p>three</p>"),
        "{fixed}"
    );
}

#[test]
fn a_div_inside_a_span_is_demoted() {
    // A <span> does not auto-close, so splitting would be invention; the <div>
    // keeps its content and loses its block status instead.
    let (_, after) = fix(&book2("<p><span>before<div>block</div>after</span></p>"));
    let fixed = ch1(&after);
    assert!(
        fixed.contains(r#"<span>before<span class="div">block</span>after</span>"#),
        "{fixed}"
    );
}

/// HTML5 gave `<caption>` and `<dt>` flow content, so a `<p>` in either is
/// correct EPUB 3 and must survive untouched. Demoting it would rewrite valid
/// markup and turn a block into an inline for no validation benefit — the same
/// mistake as applying the phrasing-only rule to an EPUB 3 `<a>`.
#[test]
fn an_epub3_caption_and_dt_may_keep_flow_content() {
    let body = concat!(
        "<table><caption><p>Table one</p></caption><tr><td>x</td></tr></table>",
        "<dl><dt><p>Term</p></dt><dd>gloss</dd></dl>"
    );
    let (outcome, after) = fix(&book3(body));
    let fixed = ch1(&after);

    assert!(
        fixed.contains("<caption><p>Table one</p></caption>"),
        "{fixed}"
    );
    assert!(fixed.contains("<dt><p>Term</p></dt>"), "{fixed}");
    assert!(
        !outcome.changes.iter().any(|c| c.contains("demoted")),
        "nothing to demote: {:?}",
        outcome.changes
    );

    // EPUB 2 is the other ruleset and the other answer: XHTML 1.1 gives both
    // elements inline content only, so there the demotion is the repair.
    let (_, after2) = fix(&book2(body));
    assert!(
        ch1(&after2).contains(r#"<caption><span class="p">Table one</span></caption>"#),
        "{}",
        ch1(&after2)
    );
}
...
#[test]
fn a_span_holding_blocks_in_a_block_context_is_promoted_to_a_div() {
    // Alice in Wonderland: <div><span id="id1"><div class="title1"><p>...
    // The span is being used as a block; in a block context, the name is the
    // only thing wrong. Its id, class and content must all survive.
    let (outcome, after) = fix(&book2(
        r#"<div><span id="id1"><div class="title1"><p>CHAPTER I</p></div></span></div>"#,
    ));
    let fixed = ch1(&after);
    assert!(
        fixed.contains(r#"<div id="id1"><div class="title1"><p>CHAPTER I</p></div></div>"#),
        "{fixed}"
    );
    assert!(
        !outcome
            .findings
            .iter()
            .any(|f| f.contains("holds block content")),
        "promotion resolves the defect, it does not report it: {:?}",
        outcome.findings
    );
    assert!(
        outcome.changes.iter().any(|c| c.contains("promoted")),
        "got {:?}",
        outcome.changes
    );
}

/// The promotion is only invisible while nothing reaches the element by its
/// name. A stylesheet that says `span` anywhere has rules that stop matching the
/// moment it becomes a `<div>`, so the repair is withheld and reported — and the
/// block inside is left alone too, since demoting *it* to a `<span>` would be
/// the same rendering change one level down.
#[test]
fn a_span_the_stylesheet_targets_by_name_is_reported_not_promoted() {
    let manifest = r#"    <item id="css" href="s.css" media-type="text/css"/>"#;
    let body = r#"<div><span class="smallcaps"><p>CHAPTER I</p></span></div>"#;
    let before = make_epub(&[
        ("META-INF/container.xml", CONTAINER.as_bytes()),
        ("OEBPS/content.opf", opf("2.0", manifest, "", "").as_bytes()),
        ("OEBPS/toc.ncx", NCX.as_bytes()),
        ("OEBPS/ch1.xhtml", doc(body).as_bytes()),
        ("OEBPS/s.css", b"span { font-variant: small-caps }"),
    ]);
    let (outcome, after) = fix(&before);
    let fixed = ch1(&after);

    assert!(
        fixed.contains(r#"<span class="smallcaps"><p>CHAPTER I</p></span>"#),
        "both the span and the block inside it must survive: {fixed}"
    );
    assert!(
        !outcome.changes.iter().any(|c| c.contains("promoted")),
        "got {:?}",
    ...
    assert!(
        outcome
            .findings
            .iter()
            .any(|f| f.contains("stylesheet has rules for <span>")),
        "and it is reported: {:?}",
        outcome.findings
    );
}
...
#[test]
fn an_epub3_anchor_may_keep_flow_content() {
    // HTML5 makes <a> transparent, so this valid EPUB 3 advertisement must not
    // be rewritten as if it were XHTML 1.1 phrasing content.
    let (_, after) = fix(&book3(
        r#"<a href="https://example.test"><div>ad</div></a>"#,
    ));
    let fixed = ch1(&after);
    assert!(
        fixed.contains(r#"<a href="https://example.test"><div>ad</div></a>"#),
        "{fixed}"
    );
}

#[test]
fn a_block_inside_a_block_that_is_already_being_repaired_is_left_alone() {
    // The <p> inside the split-out blockquote is a legal child of a block
    // element — the pass must not go on to demote it too.
    let (_, after) = fix(&book2(
        "<p>before<blockquote><p>quote</p></blockquote>after</p>",
    ));
    let fixed = ch1(&after);
    assert!(
        fixed.contains("<blockquote><p>quote</p></blockquote>"),
        "{fixed}"
    );
    assert!(
        !fixed.contains("class=\"p\""),
        "the inner <p> must survive: {fixed}"
    );
}
...
/// A book carrying an Adobe page map, with the file wherever the caller says —
/// the real ones sit at the archive root, in `OEBPS/Misc/` and in `ops/`, and
/// their hrefs are relative to themselves rather than to the NCX.
fn page_map_book(map_at: &str, pages: &str, ncx: &str) -> Vec<u8> {
    let opf = opf(
        "2.0",
        r#"    <item id="pm" href="../page-map.xml" media-type="application/oebps-page-map+xml"/>"#,
        "",
        "",
    )
    .replace("<spine toc=\"ncx\"", "<spine toc=\"ncx\" page-map=\"pm\"");
    let map = format!(
        "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n\
         <page-map xmlns=\"http://www.idpf.org/2007/opf\">{pages}</page-map>"
    );
    make_epub(&[
        ("META-INF/container.xml", CONTAINER.as_bytes()),
        ("OEBPS/content.opf", opf.as_bytes()),
        ...
        (
            "OEBPS/ch1.xhtml",
            doc(r#"<p id="page_1">a</p><p id="page_2">b</p>"#).as_bytes(),
        ),
        (map_at, map.as_bytes()),
    ])
}

/// Removing `spine/@page-map` validates and drops the only pointer to a file of
/// print page numbers. EPUB 2 has a standard spelling for those, so they move
/// into it first.
#[test]
fn an_adobe_page_map_becomes_an_ncx_page_list() {
    let (outcome, after) = fix(&page_map_book(
        "page-map.xml",
        concat!(
            r#"<page name="i" href="OEBPS/ch1.xhtml"/>"#,
            r#"<page name="1" href="OEBPS/ch1.xhtml#page_1"/>"#,
            r#"<page name="2" href="OEBPS/ch1.xhtml#page_2"/>"#,
            r#"<page name="cover" href="OEBPS/ch1.xhtml"/>"#,
            r#"<page name=" " href="OEBPS/ch1.xhtml"/>"#,
        ),
        NCX,
    ));
    let ncx = entry(&after, "OEBPS/toc.ncx");

    // `type` is the one required attribute; a digit names a page number, a
    // roman numeral is front matter, anything else is special, and a blank
    // name is no page at all.
    assert!(
        ncx.contains(r#"type="front"><navLabel><text>i</text>"#),
        "{ncx}"
    );
    assert!(
        ncx.contains(r#"type="normal" value="1"><navLabel><text>1</text>"#),
        "{ncx}"
    );
    assert!(
        ncx.contains(r#"type="special"><navLabel><text>cover</text>"#),
        "{ncx}"
    );
    assert_eq!(ncx.matches("<pageTarget").count(), 4, "{ncx}");

    // The href was relative to the page map at the archive root; the NCX is a
    // directory down, so it has to be re-expressed.
    assert!(
        ncx.contains(r#"<content src="ch1.xhtml#page_1"/>"#),
        "{ncx}"
    );
    // playOrder is never written: the only way to get it wrong is to collide
    // with the navMap, and omitting it is clean.
    let list = &ncx[ncx.find("<pageList").unwrap()..];
    assert!(!list.contains("playOrder"), "{ncx}");
    assert!(
        ncx.find("</navMap>").unwrap() < ncx.find("<pageList").unwrap(),
        "{ncx}"
    );

    // Only then does the attribute go, and the map itself is left alone.
    assert!(
        !entry(&after, "OEBPS/content.opf").contains("page-map=\"pm\""),
        "{}",
        entry(&after, "OEBPS/content.opf")
    );
    assert!(has(&after, "page-map.xml"), "the page map is not deleted");
    assert!(
        outcome
            .changes
            .iter()
            .any(|c| c.contains("moved 4 print page number")),
        "got {:?}",
        outcome.changes
    );
}

/// The pair `(type, value)` has to be unique, and only a numbered page carries
/// a value — so a page name repeated in the map must not be written twice.
#[test]
fn a_page_number_appearing_twice_is_written_once() {
    let (_, after) = fix(&page_map_book(
        "page-map.xml",
        concat!(
            r#"<page name="1" href="OEBPS/ch1.xhtml#page_1"/>"#,
            r#"<page name="1" href="OEBPS/ch1.xhtml#page_2"/>"#,
        ),
        NCX,
    ));
    let ncx = entry(&after, "OEBPS/toc.ncx");
    assert_eq!(ncx.matches("<pageTarget").count(), 1, "{ncx}");
}

/// A book already carrying the standard spelling is the common case and says
/// nothing — unless the page map knows pages the `<pageList>` does not, which
/// is a real *Oathbringer* and worth a person's time.
#[test]
fn an_existing_page_list_is_only_reported_when_the_map_adds_to_it() {
    let with_list =
        |targets: &str| NCX.replace("</ncx>", &format!("<pageList>{targets}</pageList>\n</ncx>"));
    let target = |n: &str| {
        format!(
            r#"<pageTarget id="t{n}" type="normal" value="{n}"><navLabel><text>{n}</text></navLabel><content src="ch1.xhtml"/></pageTarget>"#
        )
    };
    let pages = concat!(
        r#"<page name="1" href="OEBPS/ch1.xhtml#page_1"/>"#,
    ...
    let (quiet, _) = fix(&page_map_book(
        "page-map.xml",
        pages,
        &with_list(&format!("{}{}", target("1"), target("2"))),
    ));
    assert!(
    ...
    let (loud, after) = fix(&page_map_book(
        "page-map.xml",
        pages,
        &with_list(&target("1")),
    ));
    assert!(
        loud.findings
            .iter()
            .any(|f| f.contains("names 1 page(s) it does not") && f.contains("[2]")),
        "got {:?}",
        loud.findings
    );
    // Nothing was merged: which pagination is right is a judgement.
    assert_eq!(
...
/// The Penguin *Iliad* shape: the guide points at the cover *image*, and the
/// page holding that image is sitting in the spine. The landmark is recoverable,
/// so it is recovered rather than deleted — `type` and `title` intact.
#[test]
fn a_guide_reference_to_an_image_is_repointed_at_the_page_that_shows_it() {
    let guide = r#"  <guide>
...
"#;
    ...
    let (outcome, after) = fix(&make_epub(&[
        ...
        (
            "OEBPS/cover.xhtml",
            doc(r#"<div><img src="cover.jpg" alt="Cover"/></div>"#).as_bytes(),
        ),
    ...
    assert!(
        outcome
            .changes
            .iter()
            .any(|c| c.contains("repointed 1 guide")),
        "got {:?}",
        outcome.changes
    );
    assert!(
        !outcome.changes.iter().any(|c| c.contains("removed")),
        "nothing was lost, so nothing is reported as removed: {:?}",
        outcome.changes
    );
    assert!(
        fixed.contains(
            r#"<reference type="other.ms-coverimage-standard" title="Cover" href="cover.xhtml"/>"#
        ),
        "{fixed}"
    );
}

/// Uniqueness is what makes the retarget a derivation. Two pages show the plate,
/// so there is no way to tell which the guide meant — and neither branch fits:
/// repointing would be a guess, removing would throw away a recoverable entry.
/// The book keeps its OPF-032 and a person is told why.
#[test]
fn an_image_shown_by_two_pages_is_not_guessed_at() {
    let guide = r#"  <guide>
    <reference type="other.ms-thumbimage" title="Thumb" href="plate.jpg"/>
    <reference type="text" title="Text" href="ch1.xhtml"/>
  </guide>
"#;
    let opf = opf(
        "2.0",
        concat!(
            "    <item id=\"plate\" href=\"plate.jpg\" media-type=\"image/jpeg\"/>\n",
    ...
    let (outcome, after) = fix(&make_epub(&[
        ...
        (
            "OEBPS/ch1.xhtml",
            doc(r#"<div><img src="plate.jpg" alt="p"/></div>"#).as_bytes(),
        ),
        (
            "OEBPS/ch2.xhtml",
            doc(r#"<div><img src="plate.jpg" alt="p"/></div>"#).as_bytes(),
        ),
        ("OEBPS/plate.jpg", b"\xFF\xD8\xFF\xE0"),
    ]));
    let fixed = entry(&after, "OEBPS/content.opf");
    ...
    assert!(
        outcome
            .findings
            .iter()
            .any(|f| f.contains("2 pages show that file")
                && f.contains("ch1.xhtml")
                && f.contains("ch2.xhtml")),
        "the candidates are named so the choice is a glance: {:?}",
        outcome.findings
    );
    assert!(
        !outcome.changes.iter().any(|c| c.contains("guide")),
        "the entry is neither repointed nor removed: {:?}",
        outcome.changes
    );
    assert!(
        fixed.contains(r#"<reference type="other.ms-thumbimage" title="Thumb" href="plate.jpg"/>"#),
        "{fixed}"
    );
    // Neither page loses the picture over a package-level defect.
    assert!(entry(&after, "OEBPS/ch1.xhtml").contains("plate.jpg"));
    assert!(entry(&after, "OEBPS/ch2.xhtml").contains("plate.jpg"));
}

/// A retarget that lands on a `type`/document pair the guide already has would
/// be a duplicate of an entry that is already doing the job, so the bad one is
/// removed instead — and the cover is written down where EPUB 2 keeps it first.
#[test]
fn a_retarget_onto_an_entry_already_there_removes_instead() {
    let guide = r#"  <guide>
...
"#;
    ...
    assert_eq!(fixed.matches(r#"type="cover""#).count(), 1, "{fixed}");
    assert!(fixed.contains(r#"href="cover.xhtml"/>"#), "{fixed}");
    assert!(
        fixed.contains(r#"<meta name="cover" content="cov"/>"#),
        "{fixed}"
    );
}

/// A book that already says which image is its cover does not get a second
/// declaration bolted on beside the first.
#[test]
fn an_existing_cover_declaration_is_left_as_the_only_one() {
    let guide = r#"  <guide>
    <reference type="cover" title="Cover" href="cover.jpg"/>
    <reference type="text" title="Text" href="ch1.xhtml"/>
  </guide>
"#;
    ...
    assert_eq!(fixed.matches(r#"name="cover""#).count(), 1, "{fixed}");
}
...
/// A fragment that brought its own prolog. The wrapper writes an XML
/// declaration of its own, so carrying the fragment's into `<div>` is not a
/// wrapper but a new and worse defect: measured, `<div><?xml version="1.0"?>`
/// is `FATAL(RSC-016) The processing instruction target matching
/// "[xX][mM][lL]" is not allowed`. One error in, one fatal out — and reported
/// as a repair, because the pass guard asked quick-xml, which reads a late
/// declaration as an ordinary processing instruction.
#[test]
fn a_fragment_does_not_carry_its_prolog_into_the_wrapper() {
    let (_, after) = fix(&book_raw(
        "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n\
         <!DOCTYPE html>\n\
         <img alt=\"Image\" src=\"cover.gif\" />",
    ));
    let got = ch1(&after);

    assert!(
        got.contains(r#"<div><img alt="Image" src="cover.gif" /></div>"#),
        "got {got}"
    );
    // Exactly one declaration, and it is the one the wrapper wrote.
    assert_eq!(got.matches("<?xml").count(), 1, "got {got}");
    assert!(got.starts_with("<?xml"), "got {got}");
    assert!(!got.contains("<!DOCTYPE"), "got {got}");
}

/// The guard that let the above through. quick-xml is content to read a
/// declaration anywhere as a processing instruction; epubcheck calls it fatal,
/// so `well_formed` has to agree with epubcheck.
#[test]
fn a_misplaced_xml_declaration_is_not_well_formed() {
    use epubfix::markup::well_formed;
    assert!(well_formed("<?xml version=\"1.0\"?>\n<a><b/></a>").is_ok());
    assert!(well_formed("<a><div><?xml version=\"1.0\"?></div></a>").is_err());
    // An ordinary processing instruction is nobody's business but its own.
    assert!(well_formed("<a><?foo bar?></a>").is_ok());
}
...
fn a_bare_markup_fragment_becomes_a_document() {
    ...
    assert!(
        got.contains(r#"<html xmlns="http://www.w3.org/1999/xhtml""#),
        "got {got}"
    );
    assert!(got.contains("<body>"), "got {got}");
    assert!(
        got.contains(r#"<img alt="Image" src="cover.gif" />"#),
        "got {got}"
    );
    assert!(outcome.remaining.is_empty(), "got {:?}", outcome.remaining);
}
...
fn the_invented_title_comes_from_the_navigation_label() {
    let (_, after) = fix(&book_raw(r#"<img alt="Image" src="cover.gif" />"#));
    assert!(
        ch1(&after).contains("<title>A</title>"),
        "got {}",
        ch1(&after)
    );
}
...
/// package attribute is precisely what this tool must not do — so the markup
/// stays untouched and the declaration moves instead.
#[test]
fn a_body_with_block_content_is_left_alone_however_stray_its_siblings() {
    ...
    assert!(
        ...
        "the stray run is not repairable as EPUB 2, so the declaration moves: {}",
        opf_of(&after)
    );
}
...
fn book_meta(extra: &str) -> Vec<u8> {
    let opf = opf("2.0", "", "", "").replace("  </metadata>", &format!("{extra}\n  </metadata>"));
    make_epub(&[
        ("META-INF/container.xml", CONTAINER.as_bytes()),
        ("OEBPS/content.opf", opf.as_bytes()),
        ("OEBPS/toc.ncx", NCX.as_bytes()),
...
fn the_wrapper_stays_when_removing_it_would_empty_the_body() {
    let (outcome, after) = fix(&book_img(
        r#"<p class="ct-2"><img alt="" src="gone.jpg"/></p>"#,
    ));
    let got = ch1(&after);
...
fn a_link_whose_target_was_never_a_filename_says_so() {
    for (href, text) in [
        ...
        assert!(
            got.contains(&format!(r#"<a class="c">{text}</a>"#)),
            "{href}: got {got}"
        );
        assert!(
            outcome
...
fn a_comment_in_the_metadata_does_not_crash_the_run() {
    let (outcome, after) = fix(&book_meta("    <!-- calibre metadata -->\n    <dc:date/>"));
...
fn legal_head_content_is_untouched() {
    ...
    let got = ch1(&after);
    assert!(
        got.contains(r#"<meta name="author" content="me"/>"#),
        "got {got}"
    );
...
/// The Pinocchio case: a whole contents page written as href="../#ANCHOR"
/// with the anchors themselves dropped by the converter. The climb makes the
/// reference unresolvable, but the link text still names the heading.
#[test]
fn a_climbing_link_whose_anchor_is_gone_is_repointed_at_its_heading() {
    let (outcome, after) = fix(&book_toc(
        r#"<p><a href="../#CHAPTER_VI">Pinocchio's Feet Burn to Cinders</a></p>"#,
        r#"<h2 id="pb_3">PINOCCHIO'S FEET BURN TO CINDERS</h2>"#,
        r#"<h2 id="pb_4">SOMETHING ELSE</h2>"#,
    ));
...
fn an_ambiguous_heading_is_reported_rather_than_guessed_at() {
    ...
    assert!(
        ch1(&after).contains(r#"href="_Toc1""#),
        "got {}",
        ch1(&after)
    );
...
fn a_working_link_is_never_repointed_by_its_text() {
    ...
    assert!(
        ch1(&after).contains(r#"href="s2.xhtml""#),
        "got {}",
        ch1(&after)
    );
    assert!(outcome.changes.is_empty(), "got {:?}", outcome.changes);
...
fn a_self_closing_nested_anchor_becomes_a_whole_span() {
    ...
    assert!(got.contains(r#"<span id="page_viii"></span>"#), "got {got}");
...
fn a_self_closing_nested_anchor_with_no_id_leaves_nothing() {
    let (_, after) = fix(&book2(r#"<p><a href="ch1.xhtml#x"><a/>Stoicism</a></p>"#));
    let got = ch1(&after);
...
fn the_same_rule_holds_in_a_content_document() {
    ...
    assert!(
        got.contains(r#"id="x_2">three"#),
        "the innocent one stays: {got}"
    );
}
...
fn a_fragment_on_a_manifest_href_is_dropped() {
    ...
    assert!(
        opf_of(&after).contains(r#"href="i.gif""#),
        "{}",
        opf_of(&after)
    );
}
...
fn the_manifest_entry_for_the_package_document_is_removed() {
    ...
    assert!(
        outcome
            .changes
            .iter()
            .any(|c| c.contains("package document")),
        "got {:?}",
        outcome.changes
    );
    assert!(
        !opf_of(&after).contains("content.opf"),
        "{}",
        opf_of(&after)
    );
}
...
fn a_fragment_on_an_svg_is_kept() {
    ...
    assert!(
        ch1(&after).contains(r#"src="i.svg#r""#),
        "got {}",
        ch1(&after)
    );
    assert!(outcome.changes.is_empty(), "got {:?}", outcome.changes);
...
fn entries_differing_only_by_case_are_made_distinct() {
    ...
    assert!(
        ch1(&after).contains(r#"src="Pic.gif""#),
        "got {}",
        ch1(&after)
    );
}
...
fn an_anchor_name_with_no_id_becomes_the_id() {
    ...
    assert!(
        got.contains(r##"href="#x""##),
        "the link still resolves: {got}"
    );
}
    ...
    for v3 in [false, true] {
        ...
        assert!(
            outcome
                .changes
                .iter()
                .any(|c| c.contains("legacy attribute")),
            "v3={v3} got {:?}",
            outcome.changes
        );
    }
...
fn an_uppercase_keyword_value_is_lowercased() {
    let (outcome, after) = fix(&book2(r#"<table><tr><td valign="TOP">c</td></tr></table>"#));
    assert!(
        ch1(&after).contains(r#"valign="top""#),
        "got {}",
        ch1(&after)
    );
    assert!(
...
fn a_value_that_is_not_a_keyword_is_left_for_the_report() {
    ...
    assert!(
        ch1(&after).contains(r#"dir="sideways""#),
        "got {}",
        ch1(&after)
    );
    assert!(
...
#[test]
fn a_u_with_an_existing_style_gets_it_extended_not_shadowed() {
    // A second style attribute would make the document not well-formed, which
    // the rollback gate would then have to catch: better never to write it.
    let (_, after) = fix(&book2(r#"<p><u style="color: red">underlined</u></p>"#));
    let got = ch1(&after);
    assert!(
        got.contains(r#"style="color: red; text-decoration: underline""#),
        "got {got}"
    );
    assert_eq!(
        got.matches("style=").count(),
        1,
        "one style attribute: {got}"
    );
}

#[test]
fn a_center_element_becomes_a_centered_div() {
    // <center> is gone from both rulesets; its whole effect is text-align.
    for book in [
        book2("<center><p>middle</p></center>"),
        make_epub(&[
            ("META-INF/container.xml", CONTAINER.as_bytes()),
            ...
            (
                "OEBPS/ch1.xhtml",
                doc("<center><p>middle</p></center>").as_bytes(),
            ),
        ]),
    ] {
        let (outcome, after) = fix(&book);
        let got = ch1(&after);
        assert!(!got.contains("<center"), "got {got}");
        assert!(
            got.contains(r#"<div style="text-align: center"><p>middle</p></div>"#),
            "got {got}"
        );
...
fn a_dead_link_is_removed_whatever_its_rel_says() {
    ...
    assert!(
        outcome
            .changes
            .iter()
            .any(|c| c.contains("resource include")),
        "got {:?}",
        outcome.changes
    );
    assert!(
        !outcome
            .findings
            .iter()
            .any(|f| f.contains("carries content")),
...
fn each_run_is_wrapped_separately_and_blocks_are_left_where_they_are() {
    ...
    let got = ch1(&after);
    assert!(
        got.contains("<p>proper</p><div>loose verse</div><p>also proper</p>"),
        "got {got}"
    );
}
...
fn a_wrong_encoding_declaration_is_left_alone_under_epub2() {
    ...
    assert!(
        ch1(&after).contains("1999/xhtml; charset"),
        "got {}",
        ch1(&after)
    );
}
...
/// A stylesheet link carrying Word's `charset` attribute, with the stylesheet
/// present so the link itself is not a dangling resource.
fn book_with_link_charset(version: &str) -> Vec<u8> {
    let opf = opf(
        version,
        r#"    <item id="css" href="s.css" media-type="text/css"/>"#,
        "",
    ...
    let body = doc("<p>text</p>").replace(
        "<head>",
        r#"<head><link href="s.css" type="text/css" charset="UTF-8" rel="stylesheet"/>"#,
    );
    if version == "2.0" {
        make_epub(&[
            ("META-INF/container.xml", CONTAINER.as_bytes()),
            ...
            ("OEBPS/toc.ncx", NCX.as_bytes()),
            ("OEBPS/ch1.xhtml", body.as_bytes()),
            ("OEBPS/s.css", b"p { margin: 0; }".as_slice()),
        ])
    } else {
        make_epub(&[
            ("META-INF/container.xml", CONTAINER.as_bytes()),
...
/// Word writes `charset` on every stylesheet link, and epubcheck rejects it
/// under both rulesets — measured on a Calibre *Stranger* — while the
/// attribute names nothing, since a stylesheet's encoding is its own `@charset`
/// or BOM. So it is deleted and the link kept.
#[test]
fn a_charset_attribute_on_a_stylesheet_link_is_removed_under_either_version() {
    for version in ["2.0", "3.0"] {
        let (outcome, after) = fix(&book_with_link_charset(version));
        let got = ch1(&after);
        assert!(!got.contains("charset"), "{version} got {got}");
        assert!(
            got.contains(r#"rel="stylesheet""#),
            "{version}: the link itself is kept: {got}"
        );
        assert!(
            outcome
                .changes
                .iter()
                .any(|c| c.contains("charset attribute(s)")),
            "{version} got {:?}",
            outcome.changes
        );
    }
...
fn a_same_document_link_to_nothing_loses_its_href_and_keeps_its_text() {
    ...
    assert!(
        got.contains(r#"<a class="c">Isaac Asimov</a>"#),
        "got {got}"
    );
    assert!(
...
fn a_same_document_link_that_resolves_keeps_its_href() {
    ...
    assert!(
        ch1(&after).contains(r##"href="#here""##),
        "got {}",
        ch1(&after)
    );
    assert!(
...
fn a_guide_href_missing_a_directory_step_is_repointed_at_the_real_file() {
    ...
    assert!(
        opf.contains(r#"<reference href="../ch1.xhtml""#),
        "got {opf}"
    );
    assert!(
        outcome.changes.iter().any(|c| c.contains("repointed")),
...
fn a_spine_document_that_cannot_be_found_is_reported_and_never_removed() {
    ...
    assert!(opf.contains(r#"<itemref idref="ch1"/>"#), "got {opf}");
    assert!(
        opf.contains(r#"id="ch1""#),
        "the manifest entry stays: {opf}"
    );
    assert!(
        outcome
            .findings
...
fn a_dead_manifest_item_outside_the_spine_is_removed() {
    let opf = opf(
        "2.0",
        r#"    <item id="js" href="js/kobo.js" media-type="application/javascript"/>"#,
        "",
        "",
    );
    let (outcome, after) = fix(&make_epub(&[
    ...
    assert!(
        !entry(&after, "OEBPS/content.opf").contains("kobo.js"),
        "got {}",
        entry(&after, "OEBPS/content.opf")
    );
    assert!(
        outcome
            .changes
            .iter()
            .any(|c| c.contains("not in the book")),
        "got {:?}",
        outcome.changes
    );
}

/// A `<reference>` may carry a fragment and a moved one has to keep it.
/// Dropping it turns "the cover page, at this anchor" into "the cover page",
/// which validates and quietly loses the precision the landmark was for.
#[test]
fn a_moved_guide_reference_keeps_its_fragment() {
    let opf = opf(
        "2.0",
        r#"    <item id="c" href="Text/ch1.xhtml" media-type="application/xhtml+xml"/>"#,
        "",
        "  <guide><reference type=\"text\" title=\"Start\" href=\"ch1.xhtml#start\"/></guide>\n",
    );
    let (outcome, after) = fix(&make_epub(&[
        ("META-INF/container.xml", CONTAINER.as_bytes()),
        ("OEBPS/content.opf", opf.as_bytes()),
        ...
        (
            ...
            doc(r#"<p id="start">real</p>"#).as_bytes(),
        ),
    ]));
    let fixed = entry(&after, "OEBPS/content.opf");
    assert!(
        fixed.contains(r#"href="Text/ch1.xhtml#start""#),
        "got {fixed}"
    );
    assert!(
        outcome.changes.iter().any(|c| c.contains("repointed")),
...
///
/// No document shows `cover.jpg`, so there is nothing to repoint the entry at
/// and removal is the only answer left.
#[test]
fn a_guide_whose_every_entry_goes_is_removed_with_them() {
    ...
    // The entry was the only thing saying which image is the cover, so that
    // fact is written down before it goes.
    assert!(
        got.contains(r#"<meta name="cover" content="cov"/>"#),
        "got {got}"
    );
}
...
fn nav_points_without_an_id_are_given_one_and_the_rest_keep_theirs() {
    ...
    assert!(
        outcome
            .changes
            .iter()
            .any(|c| c.contains("the schema requires")),
        "got {:?}",
        outcome.changes
    );
}
...
/// `<center/>` without touching the `/>` leaves `<div/>`, which an XML parser
/// reads as an empty element and an HTML one reads as an *unclosed* start tag
/// that swallows the rest of the document. `nesting::rewrap` learned this on a
/// real `<a id="page_viii"/>`; it is the same trap one module over.
#[test]
fn a_self_closing_legacy_element_is_retagged_with_both_tags() {
    let (_, after) = fix(&make_epub(&[
        ("META-INF/container.xml", CONTAINER.as_bytes()),
        ("OEBPS/content.opf", opf("3.0", "", "", "").as_bytes()),
        (
            "OEBPS/ch1.xhtml",
            doc("<p>a</p><div><center/></div>").as_bytes(),
        ),
    ]));
    let got = ch1(&after);
    assert!(
        got.contains(r#"<div style="text-align: center"></div>"#),
        "got {got}"
    );
    assert!(!got.contains(r#"center"/>"#), "no self-closed div: {got}");
}
...
/// A split paragraph reopens with the original tag, and an `id` on it can only
/// be worn by one piece. Copying it onto every fragment traded one RSC-005 for
/// two `Duplicate ID`s — and `duplicate-ids` runs earlier, so nothing came
/// along to clean up after it.
#[test]
fn a_split_paragraph_does_not_duplicate_its_id() {
    let body = r##"<p id="intro" class="x">before<blockquote>quoted</blockquote>after</p><p><a href="#intro">to intro</a></p>"##;
    let (_, after) = fix(&make_epub(&[
        ("META-INF/container.xml", CONTAINER.as_bytes()),
        ("OEBPS/content.opf", opf("3.0", "", "", "").as_bytes()),
        ("OEBPS/ch1.xhtml", doc(body).as_bytes()),
    ...
    assert_eq!(got.matches(r#"id="intro""#).count(), 1, "got {got}");
    // The first fragment keeps it, because that is what every link already
    // resolves to; everything else about the tag is carried through.
    assert!(
        got.contains(r#"<p id="intro" class="x">before</p>"#),
        "got {got}"
    );
    assert!(got.contains(r#"<p class="x">after</p>"#), "got {got}");
}

/// A `<caption>` is the one thing that legally precedes the columns, and the
/// run has to be looked for after it. Searching from the first child stopped on
/// the caption and left the error unrepaired.
#[test]
fn a_caption_before_the_columns_does_not_stop_the_search() {
    let body = r#"<table><caption>Table 1</caption><col style="width: 54px"/><col style="width: 270px"/><tr><td>x</td></tr></table>"#;
    let (outcome, after) = fix(&make_epub(&[
        ("META-INF/container.xml", CONTAINER.as_bytes()),
        ("OEBPS/content.opf", opf("3.0", "", "", "").as_bytes()),
        ("OEBPS/ch1.xhtml", doc(body).as_bytes()),
    ...
    assert!(
        got.contains(
            r#"<caption>Table 1</caption><colgroup><col style="width: 54px"/><col style="width: 270px"/></colgroup>"#
        ),
        "got {got}"
    );
    assert!(
...
/// The wrap spans first..last, so the run must still stop at a real gap —
/// collecting every `<col>` child would pull the `<thead>` between them inside
/// the `<colgroup>`.
#[test]
fn a_gap_in_the_column_run_still_ends_it() {
    let body = r#"<table><col span="1"/><thead><tr><th>h</th></tr></thead><col span="2"/><tr><td>x</td></tr></table>"#;
    let (_, after) = fix(&make_epub(&[
        ("META-INF/container.xml", CONTAINER.as_bytes()),
        ("OEBPS/content.opf", opf("3.0", "", "", "").as_bytes()),
        ("OEBPS/ch1.xhtml", doc(body).as_bytes()),
    ...
    let got = ch1(&after);
    assert!(
        got.contains(r#"<colgroup><col span="1"/></colgroup><thead>"#),
        "got {got}"
    );
}
...
fn a_col_already_in_a_colgroup_is_left_alone() {
    ...
    assert_eq!(
        ch1(&after).matches("<colgroup>").count(),
        1,
        "got {}",
        ch1(&after)
    );
...
fn image_map_attributes_are_removed_from_an_anchor_under_epub3() {
    ...
    let got = ch1(&after);
    assert!(
        !got.contains("shape") && !got.contains("coords"),
        "got {got}"
    );
    // Everything that means something stays.
    assert!(
        got.contains(r#"<a href="ch1.xhtml" class="c">Cover</a>"#),
        "got {got}"
    );
    assert!(
...
/// A book with an image map, for the one branch no book in the library
/// exercises. Every expectation in these tests was measured against EPUB Check
/// 5.2.1 on the same shapes first.
fn map_book(map: &str) -> Vec<u8> {
    let body = format!(r##"<p><img src="plan.jpg" alt="plan" usemap="#m"/></p><p>{map}</p>"##);
    make_epub(&[
        ("META-INF/container.xml", CONTAINER.as_bytes()),
        (
            "OEBPS/content.opf",
            opf(
                "3.0",
                r#"    <item id="plan" href="plan.jpg" media-type="image/jpeg"/>"#,
                "",
                "",
            )
            .as_bytes(),
        ...
        ("OEBPS/ch1.xhtml", doc(&body).as_bytes()),
        ("OEBPS/plan.jpg", b"\xFF\xD8\xFF\xE0"),
    ])
}

/// The word the removal rests on is *outside*. Inside a `<map>` those numbers
/// are the clickable region itself, and HTML5 moved them to `<area>` rather
/// than dropping them — so they move, and the anchor keeps its text and link.
///
/// Measured: the input is two RSC-005s and the output is clean.
#[test]
fn an_image_map_hotspot_moves_onto_the_area_html5_keeps_it_on() {
    let (outcome, after) = fix(&map_book(concat!(
        r#"<map name="m" id="m">"#,
        r#"<a href="ch1.xhtml" shape="circle" coords="10,10,5">North</a>"#,
        r#"</map>"#
    )));
    let got = ch1(&after);

    // The geometry now lives where it is legal, with the co-required alt taken
    // from the anchor's own words.
    assert!(
        got.contains(
            r#"<area shape="circle" coords="10,10,5" href="ch1.xhtml" alt="North"/><a href="ch1.xhtml">North</a>"#
        ),
        "got {got}"
    );
    // And the anchor kept everything a reader can see.
    assert!(!got.contains("shape=\"circle\"></a>"), "got {got}");
    assert_eq!(got.matches("North").count(), 2, "got {got}");
    assert!(
        outcome.changes.iter().any(|c| c.contains("hotspot")),
        "got {:?}",
        outcome.changes
    );
}

/// `default` means the whole image, and coordinates on it are an error of their
/// own — so they do not travel. The older HTML 3.2 shape spellings are mapped
/// to the four HTML5 accepts rather than copied through.
#[test]
fn area_shapes_are_normalised_and_default_drops_its_coordinates() {
    let (_, after) = fix(&map_book(concat!(
        r#"<map name="m" id="m">"#,
        r#"<a href="ch1.xhtml" shape="rectangle" coords="0,0,9,9">A</a>"#,
        r#"<a href="ch1.xhtml" shape="default" coords="0,0,9,9">B</a>"#,
        r#"<a href="ch1.xhtml" coords="1,1,2,2">C</a>"#,
        r#"</map>"#
    )));
    let got = ch1(&after);

    assert!(
        got.contains(r#"<area shape="rect" coords="0,0,9,9""#),
        "{got}"
    );
    assert!(
        got.contains(r#"<area shape="default" href="ch1.xhtml" alt="B"/>"#),
        "default carries no coords: {got}"
    );
    // No shape at all is a rect, which is what HTML falls back to.
    assert!(
        got.contains(r#"<area shape="rect" coords="1,1,2,2""#),
        "{got}"
    );
}

/// A shape no ruleset knows has no `<area>` to write, and inventing one would
/// only trade the old error for `value of attribute "shape" is invalid`.
#[test]
fn an_unknown_hotspot_shape_is_reported_rather_than_guessed_at() {
    let (outcome, after) = fix(&map_book(concat!(
        r#"<map name="m" id="m">"#,
        r#"<a href="ch1.xhtml" shape="trapezoid" coords="0,0">A</a>"#,
        r#"</map>"#
    )));
    let got = ch1(&after);

    assert!(!got.contains("<area"), "nothing was invented: {got}");
    assert!(got.contains(r#"shape="trapezoid""#), "left as it is: {got}");
    assert!(
        outcome
            .findings
            .iter()
            .any(|f| f.contains("trapezoid") && f.contains("<map>")),
        "got {:?}",
        outcome.findings
    );
}

/// An anchor with no destination gets neither `href` nor `alt`: measured, those
/// two are co-required on an `<area>` and having neither is the clean case.
#[test]
fn a_hotspot_with_no_destination_gets_neither_href_nor_alt() {
    let (_, after) = fix(&map_book(concat!(
        r#"<map name="m" id="m">"#,
        r#"<a shape="rect" coords="0,0,9,9" id="k">A</a>"#,
        r#"</map>"#
    )));
    let got = ch1(&after);
    assert!(
    ...
    let area = &got[got.find("<area").unwrap()..];
    assert!(
        !area[..area.find("/>").unwrap()].contains("alt="),
        "no alt without an href: {got}"
    );
}

/// `usemap` matches a map's `name` or its `id`, so copying one to the other
/// cannot break the link — and without a `name` the map is an error by itself.
#[test]
fn a_map_with_only_an_id_is_named_from_it() {
    let (outcome, after) = fix(&map_book(concat!(
        r#"<map id="m">"#,
        r#"<a href="ch1.xhtml" shape="rect" coords="0,0,9,9">A</a>"#,
        r#"</map>"#
    )));
    let got = ch1(&after);
    assert!(got.contains(r#"<map id="m" name="m">"#), "got {got}");
    assert!(
        outcome.changes.iter().any(|c| c.contains("named 1 <map>")),
        "got {:?}",
        outcome.changes
    );
}

/// XHTML 1.1 allows `shape` on an `<a>`, so an EPUB 2 image map is not this
/// fixer's business at all — including the `<map>` naming.
#[test]
fn an_epub2_image_map_is_left_completely_alone() {
    let map = concat!(
        r#"<map id="m">"#,
        r#"<a href="ch1.xhtml" shape="circle" coords="10,10,5">North</a>"#,
        r#"</map>"#
    );
    let body = format!(r##"<p><img src="plan.jpg" alt="p" usemap="#m"/></p><p>{map}</p>"##);
    let (_, after) = roundtrip_kept(&make_epub(&[
        ("META-INF/container.xml", CONTAINER.as_bytes()),
        (
            "OEBPS/content.opf",
    ...
    let got = ch1(&after);
    assert!(got.contains(r#"shape="circle""#), "got {got}");
    assert!(!got.contains("<area"), "got {got}");
    assert!(!got.contains("name=\"m\""), "got {got}");
}
...
fn a_percentage_image_width_is_left_alone_under_epub2() {
    ...
    assert!(
        ch1(&after).contains(r#"width="100%""#),
        "got {}",
        ch1(&after)
    );
}
...
fn a_title_that_has_a_name_is_left_alone() {
    let (outcome, after) = fix(&book2("<p>text</p>"));
    assert!(
        ch1(&after).contains("<title>T</title>"),
        "got {}",
        ch1(&after)
    );
    assert!(
