...
///
/// The attributes of every tag are walked for the same reason. The reader only
/// parses them on request, so a tag carrying the same attribute twice — which
/// XML forbids outright, and which is what a rename onto an occupied name
/// produces — read back as perfectly well-formed while no reader would open the
/// file. A fixer emitting one is now rolled back like any other damage.
/// Parse one tag's attributes, which is the only way the reader checks them.
///
/// Returns the message to report, so the caller can attach the position.
fn check_attrs(e: &BytesStart<'_>) -> Result<(), String> {
...
/// A processing instruction's target: everything up to the first whitespace.
fn pi_target(pi: &[u8]) -> String {
    String::from_utf8_lossy(pi.split(u8::is_ascii_whitespace).next().unwrap_or_default())
        .into_owned()
}
...
pub fn well_formed(src: &str) -> Result<(), ScanError> {
    ...
    // An XML declaration is only legal as the very first thing in a document.
    // quick-xml is happy to read a later one as an ordinary processing
    // instruction, and epubcheck is not: it is `FATAL(RSC-016) The processing
    // instruction target matching "[xX][mM][lL]" is not allowed`, which is the
    // one class of defect this whole function exists to keep out of a book.
    //
    // It was not hypothetical. `fragment-documents` wrapped a fragment that
    // opened with its own `<?xml?>` into `<div><?xml?></div>`, the pass guard
    // asked quick-xml whether the result was well-formed, quick-xml said yes,
    // and epubfix wrote an unopenable document and reported "1 fixed".
    ...
    loop {
        match reader.read_event() {
            ...
            Ok(Event::Start(e)) => {
                if let Err(message) = check_attrs(&e) {
            ...
            Ok(Event::Decl(_)) if started => {
                return Err(ScanError {
                    position: pos(&reader),
                    message: "an XML declaration appears after the start of the document, \
                              where only the prolog may hold one"
                        .to_string(),
                });
            }
            // The same thing spelled as a processing instruction. XML reserves
            // every target matching `[xX][mM][lL]`, wherever it appears.
            Ok(Event::PI(p)) if pi_target(&p).eq_ignore_ascii_case("xml") => {
                return Err(ScanError {
                    position: pos(&reader),
                    message: "<?xml ...?> is a reserved processing-instruction target".to_string(),
                });
            }
            Ok(Event::Text(t)) => {
                started |= !t.iter().all(u8::is_ascii_whitespace);
            }
            Ok(_) => {}
            Err(e) => {
                return Err(ScanError {
