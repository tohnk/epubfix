...
/// A fragment's content, with any prolog it brought along removed.
///
/// An XML declaration or a DOCTYPE is legal only *before* the root element, so
/// carrying one into `<body><div>…</div></body>` does not wrap a fragment, it
/// manufactures a worse defect than the one being repaired. Measured, on a
/// fragment whose first line is its own `<?xml version="1.0"?>`:
///
/// ```text
/// <body><div><?xml version="1.0"?>…</div></body>
///   FATAL(RSC-016) The processing instruction target matching
///                  "[xX][mM][lL]" is not allowed
/// ```
///
/// One error in, one *fatal* out, and reported as a repair — the pass guard let
/// it through because quick-xml reads a late declaration as an ordinary
/// processing instruction. [`well_formed`] now rejects that outright, so the
/// class cannot escape again; this is the repair that stops producing it.
///
/// Only a leading declaration or DOCTYPE is cut. A leading comment is legal
/// inside a `<div>` and belongs to whoever wrote it.
fn without_prolog<'a>(text: &'a str, nodes: &[Node]) -> &'a str {
    let mut cut = 0usize;
    for node in nodes {
        if node.kind != NodeKind::Other {
            break;
        }
        let raw = &text[node.span.clone()];
        let prolog = raw
            .get(..5)
        ...
        // Anything real before it means this is not a prolog at all.
        if !prolog || !text[cut..node.span.start].trim().is_empty() {
            break;
        }
        cut = node.span.end;
    }
...
impl Fixer for FragmentDocuments {
    ...
    fn apply(&self, book: &mut Book) -> Outcome {
        ...
        for doc in book.markup_names() {
            ...
            if !nodes
                .iter()
                .any(|n| n.name == "html" && n.kind != NodeKind::End)
            {
...
impl Fixer for ContentTypeMeta {
    ...
    fn apply(&self, book: &mut Book) -> Outcome {
        ...
        for doc in book.markup_names() {
            ...
            for node in nodes
                .iter()
                .filter(|n| n.name == "meta" && matches!(n.kind, NodeKind::Start | NodeKind::Empty))
            {
...
/// RSC-005: a `charset` attribute on a `<link>`.
///
/// Word writes `charset="UTF-8"` on every stylesheet link it emits, and
/// Calibre carries it through — one Calibre *Stranger* has it on both links of
/// every chapter, 25 errors from one habit. The attribute is meaningless on a
/// link in every ruleset: a stylesheet's encoding is its own `@charset` rule or
/// BOM, and the bytes are UTF-8 by construction ([`ContentTypeMeta`]'s
/// argument). Measured: the same `<link charset="UTF-8">` is RSC-005 under
/// EPUB 2 and EPUB 3 alike, so the attribute is deleted rather than converted
/// — removing it cannot change how the stylesheet is decoded.
pub struct LinkCharset;

impl Fixer for LinkCharset {
    fn name(&self) -> &'static str {
        "link-charset"
    }
    fn codes(&self) -> &'static [&'static str] {
    ...
    fn description(&self) -> &'static str {
        "remove the meaningless charset attribute Word puts on <link> elements"
    }
    ...
    fn apply(&self, book: &mut Book) -> Outcome {
        ...
        for doc in book.markup_names() {
            ...
            // Start and Empty only: an end tag has no attributes.
            for node in nodes
                .iter()
                .filter(|n| n.name == "link" && matches!(n.kind, NodeKind::Start | NodeKind::Empty))
            {
                if let Some(attr) = node.attr("charset") {
        ...
        if removed == 0 {
            ...
            Outcome::change(format!(
                "removed {removed} charset attribute(s) from <link> element(s)"
            ))
        }
    }
}
